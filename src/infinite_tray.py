...
def getWaypoints_circle(t, group):
    '''Waypoints follow a Lemniscate of Bernoulli'''
    waypoints = []
    # Start with the current pose
    print "============ Current Pose:\n", group.get_current_pose('ee_link')
...
def main():
    ...
    rospy.init_node('move_ur10', anonymous=False)
    ...
    if fraction == 1.0: 
        ...
        # rospy.loginfo("Executing plan on simulated robot in Gazebo")
    ...
    # Finish the ur_arm in the resting pose stored in the SRDF file
    group.set_named_target("resting")
    group.go(wait=False)
    rospy.sleep(2)
